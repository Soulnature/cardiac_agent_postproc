#!/usr/bin/env python3
"""
Run triage-only grouping with GPT-4o and visualize Dice differences.
...
def save_group_dice_plot(df: pd.DataFrame, output_plot: str, stats_csv: str) -> None:
    ...
    ax.set_title("Dice Distribution by Triage-Only Group (GPT-4o)")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Triage-only grouping with GPT-4o + Dice visualization")
    parser.add_argument("--config", required=True, help="YAML config path (e.g., config/azure_openai_medrag.yaml)")
    parser.add_argument(
        "--output_csv",
        default="batch_eval_results/triage_only_gpt4o_grouping.csv",
        help="Output CSV path for triage-only grouping",
    )
    parser.add_argument(
        "--output_plot",
        default="batch_eval_results/triage_only_gpt4o_dice_diff.png",
        help="Output plot path for Dice group comparison",
    )
    parser.add_argument(
        "--stats_csv",
        default="batch_eval_results/triage_only_gpt4o_dice_stats.csv",
        help="Output CSV path for Dice summary stats by group",
    )
    parser.add_argument(
        "--debug_dir",
        default="batch_eval_results/triage_only_gpt4o_debug",
    ...
    for ctx in tqdm(all_cases, desc="Triage-only GPT-4o"):
        t0 = time.time()
